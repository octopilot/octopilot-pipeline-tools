"""Load op-run.yaml for preconfigured ports, env, and volumes for `op run`."""
...
RUN_CONFIG_FILENAME = "op-run.yaml"
...
def load_run_config(cwd: Path) -> dict[str, Any]:
    """
    Load op-run.yaml from cwd. Returns a dict with:
    ...
    try:
        ...
        raise RuntimeError("PyYAML required to read op-run.yaml. pip install pyyaml") from None
...
def get_run_options_for_context(
    ...
    *,
    ...
    Merges op-run.yaml context entry with defaults (ports 8080:8080, env PORT=8080).
    """
    if config is None:
...
def get_default_repo_and_tag_for_run(cwd: Path, config: dict[str, Any] | None = None) -> tuple[str, str]:
    """Return (default_repo, tag) for op run. Uses op-run.yaml, then falls back to localhost:5001 and latest."""
    if config is None:
        config = load_run_config(cwd)

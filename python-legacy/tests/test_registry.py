...
def test_load_registry_file_local_and_ci(tmp_path: Path) -> None:
    ...
    assert data["local"] == "localhost:5001"
...
def test_get_push_registries_auto_local(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    (tmp_path / REGISTRY_FILENAME).write_text("local: localhost:5001\nci:\n  - ghcr.io/org\n")
    monkeypatch.delenv("GITHUB_ACTIONS", raising=False)
    assert get_push_registries(repo_root=tmp_path, destination="auto", in_ci=False) == ["localhost:5001"]
...
def test_get_push_registries_destination(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    (tmp_path / REGISTRY_FILENAME).write_text("local: localhost:5001\nci:\n  - ghcr.io/org\n  - gcr.io/p\n")
    assert get_push_registries(repo_root=tmp_path, destination="local") == ["localhost:5001"]
    ...
    assert "localhost:5001" in all_reg
    assert "ghcr.io/org" in all_reg


def test_get_default_repo_from_registry(tmp_path: Path) -> None:
    (tmp_path / REGISTRY_FILENAME).write_text("local: localhost:5001\nci:\n  - ghcr.io/org\n")
    assert get_default_repo_from_registry(repo_root=tmp_path, destination="local") == "localhost:5001"
